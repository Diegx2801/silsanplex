...
import { DetalleCliente } from '@/modulos/clientes/componentes/DetalleCliente'
...
import { consultarDni } from '@/modulos/clientes/servicios/dniLookupService'
...
export function ClientesPage() {
  ...
  return <div className="space-y-8">
    ...
    {abierto ? <DialogoCliente key={seleccionado?.id ?? 'nuevo'} abierto={abierto} cliente={seleccionado} alCambiarApertura={setAbierto} alGuardar={guardar} alConsultarRuc={consultarRuc} alConsultarDni={consultarDni} alRestaurarFoco={() => disparador.current?.focus()} /> : null}
    ...
    {clienteDetalle ? <DetalleCliente abierto cliente={clienteDetalle} alCambiarApertura={(abierto) => { if (!abierto) setClienteDetalle(null) }} alRestaurarFoco={() => disparadorDetalle.current?.focus()} /> : null}
