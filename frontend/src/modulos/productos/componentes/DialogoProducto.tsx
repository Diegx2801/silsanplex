...
import { afectacionesIgv, esquemaProducto, productoInicial, type DatosProducto, type Producto, type ResultadoGuardadoProducto, type UnidadMedida } from '@/modulos/productos/modelo/producto'
import { GestorImagenesProducto } from '@/modulos/productos/componentes/GestorImagenesProducto'
...
interface DialogoProductoProps {
  ...
  alGuardar: (datos: DatosProducto, productoId?: string, imagenPrincipal?: File | null) => Promise<ResultadoGuardadoProducto>
...
export function DialogoProducto({ abierto, producto, unidadesMedida, alCambiarApertura, alGuardar, alRestaurarFoco }: DialogoProductoProps) {
  ...
  const guardar = async (datos: DatosProducto) => {
    ...
    const resultado = await alGuardar(
      { ...datos, unidadMedida: unidad?.nombre ?? datos.unidadMedida },
      producto?.id,
      producto ? null : imagenPrincipal,
    )
    if (resultado.error) {
      setError('root.server', { message: resultado.error })
  ...
  return <DialogPrimitive.Root open={abierto} onOpenChange={alCambiarApertura}><DialogPrimitive.Portal><DialogPrimitive.Overlay className="fixed inset-0 z-40 bg-foreground/25" /><DialogPrimitive.Content className="fixed inset-0 z-50 m-auto flex h-dvh w-full flex-col bg-background shadow-xl outline-none sm:h-[min(90vh,860px)] sm:max-w-3xl sm:rounded-lg sm:border" onCloseAutoFocus={(evento) => { evento.preventDefault(); alRestaurarFoco() }}>
    ...
    <form ref={formularioRef} id="formulario-producto" className="min-h-0 flex-1 overflow-y-auto" onSubmit={handleSubmit(guardar)}>
      ...
      <section className="border-t px-5 py-6 sm:px-7"><div className="mb-5"><h2 className="font-semibold">Información comercial</h2><p className="mt-1 text-sm text-muted-foreground">Los precios son finales por unidad base e incluyen IGV cuando corresponda. El costo real se determinará desde Compras.</p></div><div className="grid gap-5 sm:grid-cols-2"><CampoTexto etiqueta="Precio de venta final (S/)" inputMode="decimal" placeholder="0.00" error={errors.precioVenta?.message} ayuda="Incluye IGV cuando corresponda." {...register('precioVenta')} /><CampoTexto etiqueta="Precio mínimo final (S/)" inputMode="decimal" placeholder="Sin mínimo" error={errors.precioMinimo?.message} ayuda="Incluye IGV cuando corresponda. Déjalo vacío para no establecer mínimo." {...register('precioMinimo')} /><div><label htmlFor="afectacion-igv" className="field-label">Afectación de IGV</label><select id="afectacion-igv" className="field-control" {...register('afectacionIgv')}>{afectacionesIgv.map((opcion) => <option key={opcion.valor || 'sin-definir'} value={opcion.valor}>{opcion.etiqueta}</option>)}</select></div></div></section>
      <section className="border-t px-5 py-6 sm:px-7"><div className="mb-4"><h2 className="font-semibold">Imágenes</h2><p className="mt-1 text-sm text-muted-foreground">La imagen principal facilita reconocer el producto en el catálogo.</p></div>{producto ? <GestorImagenesProducto producto={producto} /> : <div className="rounded-md border border-dashed p-4"><span className="field-label">Imagen principal opcional</span><div className="mt-2 flex flex-col gap-3 sm:flex-row sm:items-center"><input ref={imagenInputRef} id="imagen-principal-producto" type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={(evento) => setImagenPrincipal(evento.target.files?.[0] ?? null)} /><label htmlFor="imagen-principal-producto" className="inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-md border bg-background px-4 text-sm font-medium hover:bg-muted"><ImagePlus aria-hidden="true" className="size-4" />Seleccionar imagen</label><span className="min-w-0 flex-1 truncate text-sm text-muted-foreground">{imagenPrincipal?.name ?? 'Ningún archivo seleccionado'}</span>{imagenPrincipal ? <Button type="button" variant="ghost" size="sm" onClick={() => { setImagenPrincipal(null); if (imagenInputRef.current) imagenInputRef.current.value = '' }}><Trash2 aria-hidden="true" />Quitar</Button> : null}</div><p className="mt-2 text-xs text-muted-foreground">JPG, PNG o WEBP; máximo 5 MB.</p></div>}</section>
      ...
      <section className="border-t px-5 py-6 sm:px-7"><h2 className="mb-2 font-semibold">Control operativo</h2>{tipo === 'good' ? <><OpcionBinaria etiqueta="Controlar por lote" descripcion="Solicita lote al ingresar inventario." {...register('controlLote')} /><OpcionBinaria etiqueta="Controlar vencimiento" descripcion="Solicita vencimiento en las entradas." {...register('controlVencimiento')} /><OpcionBinaria etiqueta="Controlar por número de serie" descripcion="Solicita una serie única para equipos que requieran trazabilidad individual." {...register('serialControl')} /><OpcionBinaria etiqueta="Venta con receta" descripcion="Marca esta condición comercial." {...register('ventaReceta')} /></> : <p className="py-3 text-sm text-muted-foreground">Los servicios no generan stock, lotes, vencimientos ni números de serie.</p>}<OpcionBinaria etiqueta="Activo" descripcion="Permite usar este registro en operaciones nuevas." {...register('activo')} /></section>
    </form>
