...
export function DireccionesEntrega({ fieldArray, errors, register, getValues, setValue, clearErrors, control }: Props) {
  ...
  return <fieldset aria-labelledby="direcciones-entrega-titulo" className="space-y-3 border-t pt-5 sm:col-span-2"><div className="flex flex-wrap items-center justify-between gap-3"><h3 id="direcciones-entrega-titulo" className="font-semibold">Direcciones de entrega</h3><Button type="button" variant="outline" onClick={agregar} disabled={fields.length >= 20}><Plus /> Agregar</Button></div><p className="text-sm text-muted-foreground">Registra hasta 20 direcciones. Si agregas una, debes mantener una como principal.</p>
