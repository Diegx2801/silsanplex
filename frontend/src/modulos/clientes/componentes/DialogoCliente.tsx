...
export function DialogoCliente({ abierto, cliente, alCambiarApertura, alGuardar, alConsultarRuc, alConsultarDni, alRestaurarFoco }: Props) {
  ...
  const [rucConsultado, setRucConsultado] = useState<string | null>(() => (
    cliente?.tipoDocumento === 'ruc' && cliente.fechaConsultaSunat && cliente.fuenteDatosFiscales
      ? cliente.numeroDocumento
      : null
  ))
  const [dniConsultado, setDniConsultado] = useState<string | null>(() => (
    cliente?.tipoDocumento === 'dni' && cliente.fechaConsultaSunat && cliente.fuenteDatosFiscales
      ? cliente.numeroDocumento
      : null
  ))
  ...
  const limpiarDatosFiscales = () => {
    setValue('nombreRazonSocial', '')
    setValue('direccion', '')
    setValue('ubigeo', '')
    setValue('estadoSunat', '')
    setValue('condicionDomicilio', '')
  }
  const invalidarConsulta = (limpiarFiscal: boolean) => {
    const limpiarNombre = nombreAutocompletado || Boolean(rucConsultado || dniConsultado)
    consultaId.current += 1
    setRucConsultado(null)
    setDniConsultado(null)
    setNombreAutocompletado(false)
    setValue('fuenteDatosFiscales', '')
    setValue('fechaConsultaSunat', null)
    setConsultando(false)
    if (limpiarFiscal) limpiarDatosFiscales()
  ...
  const consultarRuc = async () => {
    ...
    try {
      ...
      if (id !== consultaId.current || tipoDocumento !== 'ruc' || getValues('numeroDocumento').trim() !== ruc) {
        setMensajeConsulta('El número de RUC cambió durante la consulta. Vuelve a consultar para obtener datos del nuevo RUC.')
        return
      }
      ...
      setMensajeConsulta(`Datos tributarios consultados el ${new Intl.DateTimeFormat('es-PE', { dateStyle: 'short', timeStyle: 'short' }).format(new Date(resultado.checkedAt))}.`)
    } catch (error) {
      if (id === consultaId.current) setMensajeConsulta(error instanceof Error ? error.message : 'No se pudo consultar el RUC. Completa los datos manualmente.')
    } finally {
      if (id === consultaId.current) setConsultando(false)
    }
  }
  const consultarDni = async () => {
    const dni = getValues('numeroDocumento').trim()
    if (!/^\d{8}$/.test(dni)) {
      setError('numeroDocumento', { message: 'El DNI debe contener 8 dígitos' })
      return
    }
    clearErrors('numeroDocumento')
    const id = ++consultaId.current
    ...
    setMensajeConsulta('')
    try {
      const resultado = await alConsultarDni(dni)
      if (id !== consultaId.current || tipoDocumento !== 'dni' || getValues('numeroDocumento').trim() !== dni) {
        setMensajeConsulta('El número de DNI cambió durante la consulta. Vuelve a consultar para obtener datos del nuevo DNI.')
        return
      }
      setRucConsultado(null)
      setDniConsultado(dni)
      setNombreAutocompletado(true)
      setValue('nombreRazonSocial', resultado.fullName, { shouldValidate: true, shouldDirty: true })
      setValue('fuenteDatosFiscales', resultado.source)
      setValue('fechaConsultaSunat', resultado.checkedAt)
      setMensajeConsulta(`Nombre consultado en RENIEC el ${new Intl.DateTimeFormat('es-PE', { dateStyle: 'short', timeStyle: 'short' }).format(new Date(resultado.checkedAt))}.`)
    } catch (error) {
      if (id === consultaId.current) setMensajeConsulta(error instanceof Error ? error.message : 'No se pudo consultar el DNI. Completa el nombre manualmente.')
    } finally {
      if (id === consultaId.current) setConsultando(false)
    }
  }
  ...
  return <DialogPrimitive.Root open={abierto} onOpenChange={alCambiarApertura}><DialogPrimitive.Portal>
    ...
    <DialogPrimitive.Content className="fixed start-1/2 top-1/2 z-50 max-h-[92svh] w-[calc(100%-2rem)] max-w-4xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto border bg-background shadow-xl outline-none" onCloseAutoFocus={(e) => { e.preventDefault(); alRestaurarFoco() }}>
      ...
      <form id="formulario-cliente" className="grid gap-5 px-5 py-6 sm:grid-cols-2 sm:px-7" onChange={() => clearErrors('root')} onSubmit={handleSubmit(guardar, manejarErrores)}>
        ...
        <Campo label="Persona de contacto" error={errors.contacto?.message} errorId="cliente-contacto-error"><input autoComplete="name" className="field-control" aria-invalid={Boolean(errors.contacto)} aria-describedby={errors.contacto ? 'cliente-contacto-error' : undefined} {...register('contacto')} /></Campo>
        <Campo label="Teléfono" error={errors.telefono?.message} errorId="cliente-telefono-error"><input type="tel" inputMode="tel" autoComplete="tel" maxLength={30} placeholder="+51 999 888 777" className="field-control" aria-invalid={Boolean(errors.telefono)} aria-describedby={errors.telefono ? 'cliente-telefono-error' : undefined} {...register('telefono')} /></Campo>
        <Campo label="Correo" error={errors.email?.message} errorId="cliente-email-error" ancho><input type="email" autoComplete="email" className="field-control" aria-invalid={Boolean(errors.email)} aria-describedby={errors.email ? 'cliente-email-error' : undefined} {...register('email')} /></Campo>
        <Campo label="Dirección fiscal" error={errors.direccion?.message} errorId="cliente-direccion-error"><input autoComplete="street-address" readOnly={datosFiscalesBloqueados} title={datosFiscalesBloqueados ? 'Dato obtenido de SUNAT' : undefined} className={claseCampoBloqueado(datosFiscalesBloqueados)} aria-invalid={Boolean(errors.direccion)} aria-describedby={errors.direccion ? 'cliente-direccion-error' : undefined} {...register('direccion')} /></Campo>
        ...
        <DireccionesEntrega fieldArray={direcciones} errors={errors} register={register} control={control} getValues={getValues} setValue={setValue} clearErrors={clearErrors} />
