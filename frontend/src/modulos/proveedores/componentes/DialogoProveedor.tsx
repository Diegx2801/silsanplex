...
export function DialogoProveedor({ abierto, proveedor, alCambiarApertura, alGuardar, alConsultarRuc, alRestaurarFoco }: Props) {
  ...
  return <DialogPrimitive.Root open={abierto} onOpenChange={alCambiarApertura}><DialogPrimitive.Portal>
    ...
    <DialogPrimitive.Content className="fixed start-1/2 top-1/2 z-50 max-h-[92svh] w-[calc(100%-2rem)] max-w-4xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto border bg-background shadow-xl outline-none" onCloseAutoFocus={(evento) => { evento.preventDefault(); alRestaurarFoco() }}>
      ...
      <form id="formulario-proveedor" className="grid gap-5 px-5 py-6 sm:grid-cols-2 sm:px-7" onSubmit={handleSubmit(guardar)} noValidate>
        ...
        <Campo label="Razón social o nombre *" error={errors.razonSocial?.message} ancho><input autoFocus={Boolean(proveedor)} autoComplete="organization" aria-invalid={Boolean(errors.razonSocial)} className="field-control" {...register('razonSocial', { onChange: limpiarProcedenciaFiscal })} /></Campo>
        <Campo label="Nombre comercial" error={errors.nombreComercial?.message} ancho><input aria-invalid={Boolean(errors.nombreComercial)} className="field-control" {...register('nombreComercial')} /></Campo>
        <Campo label="Código interno" error={errors.codigo?.message}><input aria-invalid={Boolean(errors.codigo)} className="field-control uppercase" placeholder="Opcional" {...register('codigo')} /></Campo>
        <Campo label="Ubigeo fiscal" error={errors.ubigeo?.message}><input inputMode="numeric" maxLength={6} aria-invalid={Boolean(errors.ubigeo)} className="field-control" {...register('ubigeo', { onChange: limpiarProcedenciaFiscal })} /></Campo>
        <Campo label="Dirección fiscal" error={errors.direccion?.message} ancho><input autoComplete="street-address" aria-invalid={Boolean(errors.direccion)} className="field-control" {...register('direccion', { onChange: limpiarProcedenciaFiscal })} /></Campo>
        <Campo label="Estado SUNAT" error={errors.estadoContribuyente?.message}><input aria-invalid={Boolean(errors.estadoContribuyente)} className="field-control" placeholder="Ej. ACTIVO" {...register('estadoContribuyente', { onChange: limpiarProcedenciaFiscal })} /></Campo>
        <Campo label="Condición del domicilio" error={errors.condicionDomicilio?.message}><input aria-invalid={Boolean(errors.condicionDomicilio)} className="field-control" placeholder="Ej. HABIDO" {...register('condicionDomicilio', { onChange: limpiarProcedenciaFiscal })} /></Campo>
        <fieldset className="grid gap-5 border-t pt-5 sm:col-span-2 sm:grid-cols-2"><legend className="mb-4 font-semibold">Contacto principal</legend><Campo label="Persona de contacto" error={errors.contacto?.message}><input autoComplete="name" aria-invalid={Boolean(errors.contacto)} className="field-control" {...register('contacto')} /></Campo><Campo label="Cargo" error={errors.cargoContacto?.message}><input aria-invalid={Boolean(errors.cargoContacto)} className="field-control" {...register('cargoContacto')} /></Campo><Campo label="Teléfono" error={errors.telefono?.message}><input inputMode="tel" autoComplete="tel" aria-invalid={Boolean(errors.telefono)} className="field-control" {...register('telefono')} /></Campo><Campo label="Correo" error={errors.email?.message}><input type="email" autoComplete="email" aria-invalid={Boolean(errors.email)} className="field-control" {...register('email')} /></Campo></fieldset>
        <fieldset className="grid gap-5 border-t pt-5 sm:col-span-2 sm:grid-cols-2"><legend className="mb-4 font-semibold">Condición comercial</legend><Campo label="Condición de pago"><select className="field-control" {...register('condicionCredito', { onChange: (evento) => { if (evento.target.value === 'contado') setValue('diasCredito', '0', { shouldValidate: true }) } })}><option value="contado">Contado</option><option value="credito">Crédito</option></select></Campo><Campo label="Días de crédito" error={errors.diasCredito?.message}><input inputMode="numeric" disabled={condicionCredito === 'contado'} aria-invalid={Boolean(errors.diasCredito)} className="field-control disabled:bg-muted" {...register('diasCredito')} /></Campo></fieldset>
        <Campo label="Observaciones" error={errors.observaciones?.message} ancho><textarea rows={3} aria-invalid={Boolean(errors.observaciones)} className="field-control py-2" {...register('observaciones')} /></Campo>
...
function Campo({ label, error, ancho, children }: { label: string; error?: string; ancho?: boolean; children: ReactNode }) { return <label className={ancho ? 'sm:col-span-2' : ''}><span className="field-label">{label}</span>{children}{error ? <span role="alert" className="field-error">{error}</span> : null}</label> }
