...
const mensajesDominio: Array<[string, string]> = [
  ...
  ['REPAIR_ASSIGNED_TECHNICIAN_REQUIRED', 'Asigna un técnico antes de entregar la reparación.'],
  ['REPAIR_APPROVED_TEST_REQUIRED', 'Registra al menos una prueba aprobada antes de entregar.'],
  ['REPAIR_FAILED_TEST_PRESENT', 'No se puede entregar mientras exista una prueba fallida.'],
  ['REPAIR_PENDING_PARTS', 'Cancela o consume las reservas pendientes antes de entregar.'],
...
export async function reservarParteReparacion(
  ...
  const { data, error } = await supabase.rpc('reserve_repair_part', {
    payload: {
      ...
      stock_status: datos.estadoStock,
