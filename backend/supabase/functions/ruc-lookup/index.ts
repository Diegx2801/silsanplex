...
function integerEnvironmentValue(name: string, fallback: number, min: number, max: number) {
...
Deno.serve(async (request) => {
  ...
  if (!headers) {
    return errorResponse({ code: 'ORIGIN_NOT_ALLOWED', message: 'Origen no permitido.' }, 403, {})
  }
  if (request.method === 'OPTIONS') return new Response(null, { status: 204, headers })
  if (request.method !== 'POST') {
    return errorResponse({ code: 'METHOD_NOT_ALLOWED', message: 'Método no permitido.' }, 405, headers)
  }
  ...
  try {
    ...
    if (!parsed.success) {
      return errorResponse({ code: 'RUC_INVALID', message: 'Ingresa un RUC válido de 11 dígitos.' }, 422, headers)
    }
    requestedRuc = parsed.data.ruc

    const { data: resolvedOrganization, error: organizationError } =
      await authorization.adminClient.rpc('resolve_edge_user_organization_permission', {
    ...
    if (organizationError || typeof resolvedOrganization !== 'string') {
      return errorResponse(
        { code: 'FORBIDDEN', message: 'No tienes permiso para consultar datos tributarios.' },
        403,
        headers,
      )
    }
    ...
    const rateLimit = integerEnvironmentValue('RUC_LOOKUP_RATE_LIMIT_PER_MINUTE', 30, 1, 1000)
    const { data: allowed, error: rateLimitError } = await authorization.adminClient.rpc(
    ...
    if (allowed !== true) {
      return errorResponse(
        { code: 'RUC_LOOKUP_RATE_LIMITED', message: 'Realizaste demasiadas consultas. Intenta nuevamente en un minuto.' },
        429,
        headers,
      )
    }
    ...
    const provider = new DecolectaRucProvider({
      ...
      timeoutMs: integerEnvironmentValue('RUC_LOOKUP_TIMEOUT_MS', 5_000, 1_000, 15_000),
    })
    ...
    if (error instanceof AuthorizationError) {
      return errorResponse({ code: 'UNAUTHORIZED', message: error.message }, 401, headers)
    }
    if (error instanceof ConfigurationError) {
      return errorResponse({ code: 'SERVER_NOT_CONFIGURED', message: 'El servidor no está configurado correctamente.' }, 503, headers)
    }
    if (error instanceof RucLookupError) {
      ...
      return errorResponse({ code: error.code, message: error.message }, error.status, headers)
    }
    return errorResponse(
      { code: 'RUC_LOOKUP_FAILED', message: 'No se pudo consultar el RUC. Puedes continuar con el registro manual.' },
      500,
      headers,
    )
  }
