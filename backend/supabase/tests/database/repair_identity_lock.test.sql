...
select ok(pg_get_functiondef('public.record_repair_diagnosis(jsonb)'::regprocedure) ~* 'for update', 'diagnosis locks the repair row');
select ok(pg_get_functiondef('public.save_repair_quote(jsonb)'::regprocedure) ~* 'for update', 'quote save and submit lock the repair row');
select ok(pg_get_functiondef('public.reserve_repair_part(jsonb)'::regprocedure) ~* 'for update', 'part reservation locks the repair row');
select ok(pg_get_functiondef('public.consume_repair_part(jsonb)'::regprocedure) ~* 'for update of repair', 'part consumption locks the repair row');
select ok(pg_get_functiondef('public.record_repair_test(jsonb)'::regprocedure) ~* 'for update', 'test recording locks the repair row');
select ok(pg_get_functiondef('public.record_repair_solution(jsonb)'::regprocedure) ~* 'for update', 'solution recording locks the repair row');
select ok(pg_get_functiondef('public.change_repair_status(uuid,uuid,text,text)'::regprocedure) ~* 'for update', 'status progression locks the repair row');
select ok(
  pg_get_functiondef('public.approve_repair_quote(uuid,uuid,uuid,text)'::regprocedure) ~* 'for update'
    and pg_get_functiondef('public.reject_repair_quote(uuid,uuid,uuid,text)'::regprocedure) ~* 'for update'
    and pg_get_functiondef('public.cancel_repair_part(uuid,uuid,text)'::regprocedure) ~* 'for update of repair'
    and pg_get_functiondef('public.deliver_repair(uuid,uuid,text)'::regprocedure) ~* 'for update'
    and pg_get_functiondef('public.cancel_repair(uuid,uuid,text)'::regprocedure) ~* 'for update',
  'all specialized status progression writers lock the repair row'
);
...
begin
  ...
  perform public.assign_repair('b1040000-0000-4000-8000-000000000001', 'b8040000-0000-4000-8000-000000000003', 'b2040000-0000-4000-8000-000000000003');
  perform public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000003","priority":"urgent","notes":"Administrative intake edit"}'::jsonb);
  perform public.assign_repair('b1040000-0000-4000-8000-000000000001', 'b8040000-0000-4000-8000-000000000023', 'b2040000-0000-4000-8000-000000000003');
  perform public.record_repair_solution('{"organization_id":"b1040000-0000-4000-8000-000000000001","repair_id":"b8040000-0000-4000-8000-000000000004","applied_solution":"Substantive solution"}'::jsonb);
  perform public.record_repair_diagnosis('{"organization_id":"b1040000-0000-4000-8000-000000000001","repair_id":"b8040000-0000-4000-8000-000000000005","technician_id":"b2040000-0000-4000-8000-000000000003","symptoms":"Does not start","cause_found":"Power board","recommended_solution":"Replace board"}'::jsonb);
  quote_id := public.save_repair_quote('{"organization_id":"b1040000-0000-4000-8000-000000000001","repair_id":"b8040000-0000-4000-8000-000000000006","items":[{"line_type":"labor","description":"Diagnosis","quantity":1,"unit_price":20}]}'::jsonb);
  ...
  part_id := public.reserve_repair_part('{"organization_id":"b1040000-0000-4000-8000-000000000001","repair_id":"b8040000-0000-4000-8000-000000000007","product_id":"b5040000-0000-4000-8000-000000000001","warehouse_id":"b6040000-0000-4000-8000-000000000001","location_id":"b7040000-0000-4000-8000-000000000001","quantity_requested":1}'::jsonb);
  ...
  perform public.record_repair_test('{"organization_id":"b1040000-0000-4000-8000-000000000001","repair_id":"b8040000-0000-4000-8000-000000000008","test_type":"Operation","result":"Passed","passed":true}'::jsonb);
  perform public.record_repair_solution('{"organization_id":"b1040000-0000-4000-8000-000000000001","repair_id":"b8040000-0000-4000-8000-000000000010","applied_solution":"Specialized replacement"}'::jsonb);
  perform public.change_repair_status('b1040000-0000-4000-8000-000000000001', 'b8040000-0000-4000-8000-000000000011', 'warranty', 'Warranty progression');
end;
$$;
...
select lives_ok($$
  select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000001","customer_id":"b4040000-0000-4000-8000-000000000002"}'::jsonb)
...
select lives_ok($$
  select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000002","product_id":"b5040000-0000-4000-8000-000000000002","serial_number":"  WARRANTY-SERIAL  "}'::jsonb)
...
select lives_ok($$
  select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000003","customer_id":"b4040000-0000-4000-8000-000000000002"}'::jsonb)
...
select lives_ok($$
  select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000023","customer_id":"b4040000-0000-4000-8000-000000000002"}'::jsonb)
...
select lives_ok($$
  select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000014","product_id":"b5040000-0000-4000-8000-000000000002","serial_number":"SERIAL-NEW"}'::jsonb)
...
select lives_ok($$
  select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000015","product_id":"b5040000-0000-4000-8000-000000000003","serial_number":"   "}'::jsonb)
...
select lives_ok($$
  select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000004","customer_id":"b4040000-0000-4000-8000-000000000001","product_id":"b5040000-0000-4000-8000-000000000001","serial_number":"   ","notes":"Equivalent late edit"}'::jsonb)
...
select lives_ok($$
  select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000012","priority":"high","notes":"Late general edit"}'::jsonb)
...
select lives_ok($$
  select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000019","customer_id":"b4040000-0000-4000-8000-000000000002"}'::jsonb)
...
select throws_ok(
  $$ select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000005","customer_id":"b4040000-0000-4000-8000-000000000002"}'::jsonb) $$,
...
select lives_ok($$
  select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000020","customer_id":"b4040000-0000-4000-8000-000000000002"}'::jsonb)
...
select throws_ok(
  $$ select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000006","customer_id":"b4040000-0000-4000-8000-000000000002"}'::jsonb) $$,
...
select lives_ok($$
  select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000021","customer_id":"b4040000-0000-4000-8000-000000000002"}'::jsonb)
...
select throws_ok($$ select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000005","customer_id":"b4040000-0000-4000-8000-000000000002","product_id":"b5040000-0000-4000-8000-000000000002","serial_number":"LOCKED-SERIAL"}'::jsonb) $$, 'P0001', 'REPAIR_IDENTITY_LOCKED', 'diagnostic history locks the complete identity');
select throws_ok($$ select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000006","customer_id":"b4040000-0000-4000-8000-000000000002"}'::jsonb) $$, 'P0001', 'REPAIR_IDENTITY_LOCKED', 'draft quote history locks identity');
select throws_ok($$ select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000007","customer_id":"b4040000-0000-4000-8000-000000000002"}'::jsonb) $$, 'P0001', 'REPAIR_IDENTITY_LOCKED', 'part reservation and its consumption path lock identity');
select throws_ok($$ select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000008","customer_id":"b4040000-0000-4000-8000-000000000002"}'::jsonb) $$, 'P0001', 'REPAIR_IDENTITY_LOCKED', 'test history locks identity');
select throws_ok($$ select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000009","customer_id":"b4040000-0000-4000-8000-000000000002"}'::jsonb) $$, 'P0001', 'REPAIR_IDENTITY_LOCKED', 'nonnull diagnosis locks identity');
select throws_ok($$ select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000010","customer_id":"b4040000-0000-4000-8000-000000000002"}'::jsonb) $$, 'P0001', 'REPAIR_IDENTITY_LOCKED', 'nonnull solution and SOLUTION_RECORDED lock identity');
select throws_ok($$ select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000011","customer_id":"b4040000-0000-4000-8000-000000000002"}'::jsonb) $$, 'P0001', 'REPAIR_IDENTITY_LOCKED', 'warranty reached by progression is not initial warranty');
select throws_ok($$ select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000022","customer_id":"b4040000-0000-4000-8000-000000000002"}'::jsonb) $$, 'P0001', 'REPAIR_IDENTITY_LOCKED', 'warranty without CREATED proof fails safely');
select throws_ok($$ select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000024","customer_id":"b4040000-0000-4000-8000-000000000002"}'::jsonb) $$, 'P0001', 'REPAIR_IDENTITY_LOCKED', 'status evidence away from intake locks even in an otherwise administrative event');
select results_eq(
  $$
    select repair.id, repair.customer_id, repair.product_id, repair.serial_number,
      repair.serial_control_snapshot,
...
select throws_ok($$ select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000013","customer_id":"b4040000-0000-4000-8000-000000000003","notes":"must roll back"}'::jsonb) $$, 'P0001', 'REPAIR_CUSTOMER_UNAVAILABLE', 'inactive same-tenant customer is rejected');
...
select throws_ok($$ select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000016","product_id":"b5040000-0000-4000-8000-000000000004"}'::jsonb) $$, 'P0001', 'REPAIR_PRODUCT_UNAVAILABLE', 'cross-tenant product is unavailable');
...
select throws_ok($$ select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000002","id":"b8040000-0000-4000-8000-000000000017","notes":"forbidden"}'::jsonb) $$, '42501', 'REPAIR_FORBIDDEN', 'authenticated ADMIN cannot address another tenant');
reset role;
set local role service_role;
select set_config('request.jwt.claims', '{"role":"service_role"}', true);
select throws_ok($$ select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000017","notes":"wrong tenant key"}'::jsonb) $$, 'P0001', 'REPAIR_NOT_FOUND', 'service_role still requires matching tenant and repair');
...
set local role authenticated;
select set_config('request.jwt.claims', '{"sub":"b2040000-0000-4000-8000-000000000001","role":"authenticated","session_id":"b3040000-0000-4000-8000-000000000001"}', true);
select throws_ok($$ select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000018","status":"diagnosis"}'::jsonb) $$, 'P0001', 'REPAIR_STATUS_USE_STATUS_RPC', 'P1-03 status error is preserved');
select throws_ok($$ select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000018","assigned_technician_id":"b2040000-0000-4000-8000-000000000003"}'::jsonb) $$, 'P0001', 'REPAIR_ASSIGN_USE_ASSIGN_RPC', 'P1-03 assignment error is preserved');
select throws_ok($$ select public.update_repair(jsonb_build_object('organization_id', 'b1040000-0000-4000-8000-000000000001', 'id', 'b8040000-0000-4000-8000-000000000018', 'received_at', (select received_at + interval '1 hour' from public.repairs where id = 'b8040000-0000-4000-8000-000000000018'))) $$, 'P0001', 'REPAIR_RECEIVED_AT_IMMUTABLE', 'P1-03 received_at error is preserved');
select throws_ok($$ select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000018","diagnosis":"bypass"}'::jsonb) $$, 'P0001', 'REPAIR_DIAGNOSIS_USE_DIAGNOSIS_RPC', 'P1-03 diagnosis error is preserved');
select throws_ok($$ select public.update_repair('{"organization_id":"b1040000-0000-4000-8000-000000000001","id":"b8040000-0000-4000-8000-000000000018","applied_solution":"bypass"}'::jsonb) $$, 'P0001', 'REPAIR_APPLIED_SOLUTION_USE_SOLUTION_RPC', 'P1-03 solution error is preserved');
...
select ok(
  (select metadata ? 'old_values' and metadata ? 'new_values' from public.repair_events where repair_id = 'b8040000-0000-4000-8000-000000000001' and event_type = 'UPDATED'),
  'allowed identity correction retains full general update event metadata'
);
select ok(
