...
returns uuid language sql as $$
  select public.update_order_quantities(jsonb_build_object(
    ...
    'items',(select jsonb_agg(jsonb_build_object('order_item_id',i.id,'quantity',
       case when i.product_type='service' then service_quantity else good_quantity end))
...
returns jsonb language sql as $$
  select jsonb_build_object('organization_id',o.organization_id,'order_id',o.id,
    ...
    'items',(select jsonb_agg(jsonb_build_object('order_item_id',i.id,'quantity',
       case when i.product_type='service' then i.quantity else good_quantity end) order by i.id)
       from public.order_items i
       where i.order_id=o.id and i.product_type='good')
   ) from public.orders o join public.sales s on s.order_id=o.id where o.operation_key=key;
$$;

create function pg_temp.complete_services_payload(key uuid, op uuid, service_quantity numeric)
returns jsonb language sql as $$
  select jsonb_build_object('organization_id',o.organization_id,'order_id',o.id,
    'sale_id',s.id,'operation_key',op,
    'items',(select jsonb_agg(jsonb_build_object('order_item_id',i.id,'quantity_to_complete',service_quantity) order by i.id)
       from public.order_items i where i.order_id=o.id and i.product_type='service'))
  from public.orders o join public.sales s on s.order_id=o.id where o.operation_key=key;
$$;
...
select throws_ok($t$select public.dispatch_order_from_reservations(jsonb_build_object(
  'organization_id','c1000000-0000-4000-8000-000000000001',
  'order_id',(select id from public.orders where operation_key='ce000000-0000-4000-8000-000000000001'),
  'sale_id',(select id from public.sales where order_id=(select id from public.orders where operation_key='ce000000-0000-4000-8000-000000000001')),
  'operation_key','cf000000-0000-4000-8000-000000000012',
  'items',jsonb_build_array(jsonb_build_object('order_item_id',(select id from public.order_items where order_id=(select id from public.orders where operation_key='ce000000-0000-4000-8000-000000000001')),'quantity',1))
))$t$, 'P0001', 'ORDER_DISPATCH_SERVICE_FORBIDDEN', 'el despacho fisico rechaza explicitamente servicios');
select lives_ok($t$select public.complete_order_services(pg_temp.complete_services_payload('ce000000-0000-4000-8000-000000000001','cf000000-0000-4000-8000-000000000003',1))$t$, 'completa parcialmente el servicio sin inventario');
select lives_ok($t$select public.complete_order_services(pg_temp.complete_services_payload('ce000000-0000-4000-8000-000000000001','cf000000-0000-4000-8000-000000000003',1))$t$, 'retry exacto parcial no duplica atencion');
select is((select status from public.orders where operation_key='ce000000-0000-4000-8000-000000000001'), 'confirmado'::text, 'servicio parcial no cierra pedido');
select lives_ok($t$select public.complete_order_services(pg_temp.complete_services_payload('ce000000-0000-4000-8000-000000000001','cf000000-0000-4000-8000-000000000004',1))$t$, 'completa el saldo del servicio');
...
select lives_ok($t$select public.complete_order_services(pg_temp.complete_services_payload('ce000000-0000-4000-8000-000000000002','cf000000-0000-4000-8000-000000000011',1))$t$, 'cumple el servicio del pedido mixto');
...
reset role;
select is((select count(*) from public.audit_events where action='ORDER_SERVICES_COMPLETED' and entity_id=(select id::text from public.orders where operation_key='ce000000-0000-4000-8000-000000000002')), 1::bigint, 'cumplimiento mixto deja trazabilidad del servicio');
select is((select count(*) from public.audit_events where action='ORDER_DISPATCHED' and entity_id=(select id::text from public.orders where operation_key='ce000000-0000-4000-8000-000000000002')), 2::bigint, 'despachos fisicos conservan su trazabilidad');
select * from finish();
rollback;
